...
public class Shortcut
{
    ...
    [SerializeField] private ModifierKey modifyerKey;
    ...
    private bool ModifiersHeld()
    {
        ...
        if (modifyerKey == ModifierKey.None && (Input.GetKey(KeyCode.LeftAlt) || Input.GetKey(KeyCode.LeftShift)))
            return false;

        //If the alt key isn't pressed when it should be or the shift key is also pressed, return false
        if (modifyerKey == ModifierKey.LeftAlt && (!Input.GetKey(KeyCode.LeftAlt) || Input.GetKey(KeyCode.LeftShift)))
            return false;

        //If the shift key isn't pressed when it should be or the alt key is also pressed, return false
        if (modifyerKey == ModifierKey.LeftShift && (!Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.LeftAlt)))
            return false;
